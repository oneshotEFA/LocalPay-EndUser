...
export default function StepProof() {
    ...
    try {
      ...
      if (verificationMethod === "SCREENSHOT" && file) {
        try {
          ...
          setError(
            err?.message ?? "Failed to read the screenshot. Please try again.",
          );
          return;
        }
      }
      ...
      // ── Gateway flow ──────────────────────────────────────────────────────
      // Verification done — notify the platform via our backend proxy.
      // Backend fires their webhook and returns successUrl for redirect.
  ...
  const LABELS: Record<string, { title: string; desc: string }> = {
    LINK: {
      ...
      desc: "Paste the transaction URL from your bank app.",
    },
    SMS: {
      title: "Bank SMS",
      desc: "Paste the full SMS exactly as received — do not edit it.",
    },
    SCREENSHOT: {
      title: "Screenshot",
      desc: "Upload your transaction confirmation screenshot.",
    },
  };
